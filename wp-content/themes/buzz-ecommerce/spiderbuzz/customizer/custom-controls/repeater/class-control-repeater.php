...
if( ! class_exists( 'Buzz_Ecommerce_Repeater_Control' ) ) {
    /**
     * Repeater control
     */
    class Buzz_Ecommerce_Repeater_Control extends WP_Customize_Control {
    	...
    	public function __construct( $manager, $id, $args = array() ) {
    		...
    		$this->row_label = array(
    			'type' => 'text',
    			'value' => esc_attr__( 'row', 'buzz-ecommerce' ),
    			'field' => false,
    		);
    		...
    		if ( empty( $this->button_label ) ) {
    			$this->button_label = sprintf( esc_attr__( 'Add New %s', 'buzz-ecommerce' ), $this->row_label['value'] );
    		}
    			...
    			foreach ( $this->fields as $field ) {
    				if ( isset( $field['type'] ) ){
    					if( 'color' === $field['type'] ){ 
                            ...
                            wp_enqueue_style( 'font-awesome', get_template_directory_uri(). '/assets/library/font-awesome/css/font-awesome.css', array(), '4.7' );
    					}
    				}
    			}
    		}
            wp_enqueue_script( 'buzz-ecommerce-repeater', get_template_directory_uri() . '/spiderbuzz/customizer/custom-controls//repeater/repeater.js', array( 'jquery', 'jquery-ui-core', 'jquery-ui-sortable' ), false, true );
    		wp_enqueue_style( 'buzz-ecommerce-repeater', get_template_directory_uri() . '/spiderbuzz/customizer/custom-controls//repeater/repeater.css', null );
    	}
    	...
    	protected function render_content() {
    		...
    		<?php if ( isset( $this->choices['limit'] ) ) : ?>
    			<p class="limit"><?php printf( esc_html__( 'Limit: %s rows', 'buzz-ecommerce' ), esc_html( $this->choices['limit'] ) ); ?></p>
    		<?php endif; ?>
    		...
    		<script type="text/html" class="customize-control-repeater-content">
    			...
    			<li class="repeater-row minimized" data-row="{{{ index }}}">
    				...
    				<div class="repeater-row-content">
    					<# _.each( data, function( field, i ) { #>
    						...
    						<div class="repeater-field repeater-field-{{{ field.type }}}">
    							...
    							<# } else if ( 'color' === field.type ) { #>
    						        ...
    						        <label>
    						            ...
    						            <# if ( field.description ) { #>
    						                <span class="description customize-control-description">{{{ field.description }}}</span>
    						            <# } #>
    						            <input class="color-picker-hex" type="text" maxlength="7" placeholder="<?php esc_attr_e( 'Hex Value', 'buzz-ecommerce' ); ?>"  value="{{{ field.default }}}" data-field="{{{ field.id }}}" {{ defaultValue }} />
    							...
    							<# } else if ( field.type === 'image' || field.type === 'cropped_image' ) { #>
    								...
    								<figure class="kirki-image-attachment" data-placeholder="<?php esc_attr_e( 'No Image Selected', 'buzz-ecommerce' ); ?>" >
    									<# if ( field.default ) { #>
    										<# var defaultImageURL = ( field.default.url ) ? field.default.url : field.default; #>
    										<img src="{{{ defaultImageURL }}}">
    									<# } else { #>
    										<?php esc_html_e( 'No Image Selected', 'buzz-ecommerce' ); ?>
    									<# } #>
    								</figure>
    
    								<div class="actions">
    									<button type="button" class="button remove-button<# if ( ! field.default ) { #> hidden<# } #>"><?php esc_html_e( 'Remove', 'buzz-ecommerce' ); ?></button>
    									<button type="button" class="button upload-button" data-label=" <?php esc_attr_e( 'Add Image', 'buzz-ecommerce' ); ?>" data-alt-label="<?php esc_attr_e( 'Change Image', 'buzz-ecommerce' ); ?>" >
    										<# if ( field.default ) { #>
    											<?php esc_html_e( 'Change Image', 'buzz-ecommerce' ); ?>
    										<# } else { #>
    											<?php esc_html_e( 'Add Image', 'buzz-ecommerce' ); ?>
    										<# } #>
    									</button>
    									<# if ( field.default.id ) { #>
    							...
    							<# } else if ( field.type === 'upload' ) { #>
    								...
    								<figure class="kirki-file-attachment" data-placeholder="<?php esc_attr_e( 'No File Selected', 'buzz-ecommerce' ); ?>" >
    									<# if ( field.default ) { #>
    									...
    									<# } else { #>
    										<?php esc_html_e( 'No File Selected', 'buzz-ecommerce' ); ?>
    									<# } #>
    								</figure>
    
    								<div class="actions">
    									<button type="button" class="button remove-button<# if ( ! field.default ) { #> hidden<# } #>"></button>
    									<button type="button" class="button upload-button" data-label="<?php esc_attr_e( 'Add File', 'buzz-ecommerce' ); ?>" data-alt-label="<?php esc_attr_e( 'Change File', 'buzz-ecommerce' ); ?>" >
    										<# if ( field.default ) { #>
    											<?php esc_html_e( 'Change File', 'buzz-ecommerce' ); ?>
    										<# } else { #>
    											<?php esc_html_e( 'Add File', 'buzz-ecommerce' ); ?>
    										<# } #>
    									</button>
    									<# if ( field.default.id ) { #>
    										<input type="hidden" class="hidden-field" value="{{{ field.default.id }}}" data-field="{{{ field.id }}}" >
    					...
    					<# }); #>
    					<button type="button" class="button-link repeater-row-remove"><?php esc_html_e( 'Remove', 'buzz-ecommerce' ); ?></button>
    				</div>
    			</li>
    		</script>
    		<?php
        ...
        function get_icon_list(){
    	    $icons = include wp_normalize_path( get_template_directory() . '/spiderbuzz/customizer/fontawesome.php' ); 
            if( $icons ){ ?>
