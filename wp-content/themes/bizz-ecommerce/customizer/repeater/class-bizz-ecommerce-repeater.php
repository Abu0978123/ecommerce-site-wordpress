...
/**
 * Class Bizz_Ecommerce_Repeater
 */
class Bizz_Ecommerce_Repeater extends WP_Customize_Control {
	...
	public function __construct( $manager, $id, $args = array() ) {
		...
		$this->add_field_label = esc_html__( 'Add new field', 'bizz-ecommerce' );
		if ( ! empty( $args['add_field_label'] ) ) {
			$this->add_field_label = $args['add_field_label'];
		}

		$this->boxtitle = esc_html__( 'Customizer Repeater', 'bizz-ecommerce' );
	...
	public function enqueue() {
		...
		wp_enqueue_script( 'bizz_ecommerce_customizer-repeater-script', get_template_directory_uri() . '/customizer/repeater/script.js', array( 'jquery', 'jquery-ui-draggable', 'wp-color-picker' ), '1.0.0', true );

		wp_enqueue_script( 'bizz_ecommerce_customizer-repeater-fontawesome-iconpicker', get_template_directory_uri() . '/customizer/repeater/icon-picker/icon-picker.js', array( 'jquery' ), '1.0.0', true );
				...
				<div class="customizer-repeater-general-control-repeater-container customizer-repeater-draggable">
					...
					<div class="customizer-repeater-box-content-hidden">
						...
						if ( $this->customizer_repeater_color_control == true ) {
							$this->input_control(
								array(
									'label'             => apply_filters( 'repeater_input_labels_filter', esc_html__( 'Color', 'bizz-ecommerce' ), $this->id, 'customizer_repeater_color_control' ),
						...
						if ( $this->customizer_repeater_color2_control == true ) {
							$this->input_control(
								array(
									'label'             => apply_filters( 'repeater_input_labels_filter', esc_html__( 'Color', 'bizz-ecommerce' ), $this->id, 'customizer_repeater_color2_control' ),
						...
						if ( $this->customizer_repeater_title_control == true ) {
							$this->input_control(
								array(
									'label' => apply_filters( 'repeater_input_labels_filter', esc_html__( 'Title', 'bizz-ecommerce' ), $this->id, 'customizer_repeater_title_control' ),
...
///////////// price///////////
						if ( $this->customizer_repeater_price_control == true ) {
							$this->input_control(
								array(
									'label' => apply_filters( 'repeater_input_labels_filter', esc_html__( 'Price', 'bizz-ecommerce' ), $this->id, 'customizer_repeater_price_control' ),
...
///////////// number///////////
						if ( $this->customizer_repeater_number_control == true ) {
							$this->input_control(
								array(
									'label' => apply_filters( 'repeater_input_labels_filter', esc_html__( 'Number', 'bizz-ecommerce' ), $this->id, 'customizer_repeater_number_control' ),
						...
						if ( $this->customizer_repeater_subtitle_control == true ) {
							$this->input_control(
								array(
									'label' => apply_filters( 'repeater_input_labels_filter', esc_html__( 'Subtitle', 'bizz-ecommerce' ), $this->id, 'customizer_repeater_subtitle_control' ),
						...
						if ( $this->customizer_repeater_text_control == true ) {
							$this->input_control(
								array(
									'label' => apply_filters( 'repeater_input_labels_filter', esc_html__( 'Text', 'bizz-ecommerce' ), $this->id, 'customizer_repeater_text_control' ),
						...
						if ( $this->customizer_repeater_link_control ) {
							$this->input_control(
								array(
									'label'             => apply_filters( 'repeater_input_labels_filter', esc_html__( 'Link', 'bizz-ecommerce' ), $this->id, 'customizer_repeater_link_control' ),
						...
						if ( $this->customizer_repeater_text2_control == true ) {
							$this->input_control(
								array(
									'label' => apply_filters( 'repeater_input_labels_filter', esc_html__( 'Button Text', 'bizz-ecommerce' ), $this->id, 'customizer_repeater_text2_control' ),
						...
						if ( $this->customizer_repeater_link2_control ) {
							$this->input_control(
								array(
									'label'             => apply_filters( 'repeater_input_labels_filter', esc_html__( 'Link', 'bizz-ecommerce' ), $this->id, 'customizer_repeater_link2_control' ),
						...
						if ( $this->customizer_repeater_shortcode_control == true ) {
							$this->input_control(
								array(
									'label' => apply_filters( 'repeater_input_labels_filter', esc_html__( 'Shortcode', 'bizz-ecommerce' ), $this->id, 'customizer_repeater_shortcode_control' ),
						...
						echo '>';
						esc_html_e( 'Delete field', 'bizz-ecommerce' );
						?>
						</button>
...
//number control//////////
					...
					<button type="button" class="social-repeater-general-control-remove-field button" style="display:none;">
						<?php esc_html_e( 'Delete field', 'bizz-ecommerce' ); ?>
					</button>
				</div>
			</div>
			<?php
	...
	private function icon_picker_control( $value = '', $show = '' ) {
		...
		?>
			>
			<span class="customize-control-title">
				<?php esc_html_e( 'Icon', 'bizz-ecommerce' ); ?>
			</span>
			<span class="description customize-control-description">
				<?php
				echo sprintf(
					/* translators: Fontawesome link with full list of icons available */
					esc_html__( 'Note: Some icons may not be displayed here. You can see the full list of icons at Fontawesome Website.', 'bizz-ecommerce' )
	...
	private function image_control( $value = '', $show = '' ) {
		...
		?>
			>
			<span class="customize-control-title">
				<?php esc_html_e( 'Image', 'bizz-ecommerce' ); ?>
			</span>
			<input type="text" class="widefat custom-media-url" value="<?php echo esc_attr( $value ); ?>">
			<input type="button" class="button button-secondary customizer-repeater-custom-media-button" value="<?php esc_attr_e( 'Upload Image', 'bizz-ecommerce' ); ?>" />
		</div>
		<?php
	}
	...
	private function logo_image_control( $value = '', $show = '' ) {
		...
		?>
			>
			<span class="customize-control-title">
				<?php esc_html_e( 'Logo Image', 'bizz-ecommerce' ); ?>
			</span>
			<input type="text" class="widefat custom-logo-media-url" value="<?php echo esc_attr( $value ); ?>">
			<input type="button" class="button button-secondary customizer-repeater-logo-custom-media-button" value="<?php esc_attr_e( 'Upload Image', 'bizz-ecommerce' ); ?>" />
		</div>
		<?php
	}
	...
	private function icon_type_choice( $value = 'customizer_repeater_icon' ) {
		?>
		<span class="customize-control-title">
			<?php esc_html_e( 'Image type', 'bizz-ecommerce' ); ?>
		</span>
		<select class="customizer-repeater-image-choice">
			<option value="customizer_repeater_icon" <?php selected( $value, 'customizer_repeater_icon' ); ?>><?php esc_html_e( 'Icon', 'bizz-ecommerce' ); ?></option>
			<option value="customizer_repeater_image" <?php selected( $value, 'customizer_repeater_image' ); ?>><?php esc_html_e( 'Image', 'bizz-ecommerce' ); ?></option>
			<option value="customizer_repeater_logo_image" <?php selected( $value, 'customizer_repeater_logo_image' ); ?>><?php esc_html_e( 'Logo Image', 'bizz-ecommerce' ); ?></option>
			<option value="customizer_repeater_none" <?php selected( $value, 'customizer_repeater_none' ); ?>><?php esc_html_e( 'None', 'bizz-ecommerce' ); ?></option>
		</select>
		<?php
	}
	...
	private function repeater_control( $value = '' ) {
		...
		$show_del        = 0;
		?>
		<span class="customize-control-title"><?php esc_html_e( 'Social icons', 'bizz-ecommerce' ); ?></span>
		<?php
		...
		if ( ( count( $social_repeater ) == 1 && '' === $social_repeater[0] ) || empty( $social_repeater ) ) {
			...
			<div class="customizer-repeater-social-repeater">
				<div class="customizer-repeater-social-repeater-container">
					...
					<?php get_template_part( $this->customizer_icon_container ); ?>
					<input type="text" class="customizer-repeater-social-repeater-link" placeholder="<?php esc_attr_e( 'Link', 'bizz-ecommerce' ); ?>">
					<input type="hidden" class="customizer-repeater-social-repeater-id" value="">
					<button class="social-repeater-remove-social-item" style="display:none">
						<?php esc_html_e( 'Remove Icon', 'bizz-ecommerce' ); ?>
					</button>
				</div>
				<input type="hidden" id="social-repeater-socials-repeater-colector" class="social-repeater-socials-repeater-colector" value=""/>
			</div>
			<button class="social-repeater-add-social-item button-secondary"><?php esc_html_e( 'Add Icon', 'bizz-ecommerce' ); ?></button>
			<?php
		} else {
			?>
			<div class="customizer-repeater-social-repeater">
				...
				foreach ( $social_repeater as $social_icon ) {
					...
					if ( ! empty( $social_icon['icon'] ) ) {
						...
						echo '<input type="text" class="customizer-repeater-social-repeater-link" placeholder="' . esc_attr__( 'Link', 'bizz-ecommerce' ) . '" value="';
					...
					if ( $show_del == 1 ) {
						...
						echo '">' . esc_html__( 'Remove Icon', 'bizz-ecommerce' ) . '</button>';
					echo '</div>';
				}
				?>
