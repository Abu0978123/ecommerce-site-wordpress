 ...
 */
var texture_customize_control_tabs = function ( $ ) {
	'use strict';
		...
		function () {
			...
			$( '.texture-tabs-control' ).each(
				function () {
					...
					var shownCtrls = $( this ).find( '.texture-customizer-tab > input:checked' ).data( 'controls' );
					showControls( customizerSection, shownCtrls );
				}
			);

			$( '.texture-customizer-tab > label' ).on(
				'click', function () {
					var customizerSection = $( this ).closest( '.accordion-section' );
...
texture_customize_control_tabs( jQuery );

//Function to get elemnt with attribute value
